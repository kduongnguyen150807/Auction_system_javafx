...
public class ThanhTimKiemController {
    ...
    public void initialize() {
        ...
        searchField.textProperty().addListener((obs, ov, nv) -> {
            if (ans1) {
                ...
                hideuserresults();
                KhungController.applysearchfilter(nv, categoryFilter.getValue() != null ? categoryFilter.getValue() : "All", 0, Double.MAX_VALUE);
            }
        });
        ...
        if (categoryFilter != null) {
            categoryFilter.valueProperty().addListener((obs, ov, nv) -> {
                if (!ans1) {
                    KhungController.applysearchfilter(searchField.getText(), nv != null ? nv : "All", 0, Double.MAX_VALUE);
                }
            });
        }
    }
    ...
    public void onSearchModeChanged() {
        ...
        if (ans1) {
            ...
            if (categoryFilter != null) { categoryFilter.setVisible(false); categoryFilter.setManaged(false); }
            if (minpricefield != null) { minpricefield.setVisible(false); minpricefield.setManaged(false); }
            if (maxpricefield != null) { maxpricefield.setVisible(false); maxpricefield.setManaged(false); }
            if (filterButton != null) { filterButton.setVisible(false); filterButton.setManaged(false); }
            ...
            if (categoryFilter != null) { categoryFilter.setVisible(true); categoryFilter.setManaged(true); }
            if (minpricefield != null) { minpricefield.setVisible(true); minpricefield.setManaged(true); }
            if (maxpricefield != null) { maxpricefield.setVisible(true); maxpricefield.setManaged(true); }
            if (filterButton != null) { filterButton.setVisible(true); filterButton.setManaged(true); }
            hideuserresults();
            KhungController.applysearchfilter(searchField.getText(), categoryFilter.getValue() != null ? categoryFilter.getValue() : "All", 0, Double.MAX_VALUE);
        }
    }
    ...
    private void searchusers(String ans3) {
        Request res2 = new Request(Request.searchusers, ans3);
    ...
    private HBox builduserrow(User ans3) {
        ...
        res2.setOnMouseExited(e -> res2.setStyle("-fx-background-color: transparent; -fx-cursor: hand;"));
        ...
        Label ans6 = new Label(res5 != null && !res5.isBlank() && !res5.equals(ans3.getusername()) ? res5 : (ans3.getemail() != null ? ans3.getemail() : ""));
        ...
        if (ans3.gettotalratings() > 0) {
            ...
            String ans8 = ans3.getavgrating() <= 2.0 ? "#ff4444" : (ans3.getavgrating() <= 3.0 ? "#ffaa00" : "#44cc44");
            res7.setStyle("-fx-text-fill: " + ans8 + "; -fx-font-size: 13;");
            Label res8 = new Label(String.format("%.1f (%d)", ans3.getavgrating(), ans3.gettotalratings()));
    ...
    public void applyfilter() {
        ...
        try {
            if (minpricefield != null && minpricefield.getText() != null && !minpricefield.getText().isBlank()) {
                ans4 = Double.parseDouble(minpricefield.getText());
            }
        } catch (Exception e) {}
        try {
            if (maxpricefield != null && maxpricefield.getText() != null && !maxpricefield.getText().isBlank()) {

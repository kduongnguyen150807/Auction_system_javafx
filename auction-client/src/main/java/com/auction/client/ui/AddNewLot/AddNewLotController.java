...
public class AddNewLotController {
      ...
      new Thread(
              () -> {
                try {
                  ...
                  System.arraycopy(
                      tailbytes, 0, body, headbytes.length + filebytes.length, tailbytes.length);
                  HttpRequest req =
                      HttpRequest.newBuilder()
                          .uri(
                              URI.create("https://api.cloudinary.com/v1_1/khanhdn-tk/image/upload"))
    ...
    new Thread(
            () -> {
              try {
                ...
                Request req = new Request(Request.ADD_LOT, data);
  ...
  private String normalizeDateTimeForServer(
      LocalDate date, Integer hour, Integer minute, Integer second) {

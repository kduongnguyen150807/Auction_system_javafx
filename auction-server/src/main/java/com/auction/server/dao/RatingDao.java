...
public class RatingDao {
  ...
  public boolean insertRating(Rating r) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      PreparedStatement res1 = conn.prepareStatement(sql)){
  ...
  public boolean hasRated(int itemId, int userId) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      try (ResultSet res2 = res1.executeQuery()) {
        ans = res2.next();
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public void recalcUserRating(int userId) {
    String sql = "SELECT AVG(stars) AS avg_s, COUNT(*) AS cnt FROM ratings WHERE rateduserid = ?";
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      PreparedStatement res1 = conn.prepareStatement(sql)){
      ...
      try (ResultSet res2 = res1.executeQuery()) {
        if (res2.next()) {
          ...
          try (PreparedStatement res4 = conn.prepareStatement(res3)) {
